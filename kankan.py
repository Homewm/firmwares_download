...
class MyFTP(FTP):
    ...
    def download(self, remoteHost, remotePort, loginname, loginpassword, remotePath, localPath):
        ...
        fsize = ftp.size(remotefile)        ###无法获得文件的大小，这个针对的是自己的服务器，我们存在这些权限的时候
...
# 下面是使用代码
ftpClient = MyFTP()
#host = 'ftp2.dlink.com', user = 'anonymous', passwd = 'mozilla@example.com'
ftpClient.download('ftp2.dlink.com', 21, 'anonymous', 'mozilla@example.com', 'PRODUCTS/', 'E:\\dlink_by')
